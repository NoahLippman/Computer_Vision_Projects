...
def trim_video_for_model(input_path, output_path, vertical_crop, time_crop):
    ...
    Does not change the fps and all other formatting

    Args:
        input_path (str): path to the video file
        output_path (str): path to write the trimmed file to

    Returns:
        list: [The frame in which the new video starts, the new video path]
    """
...
def estimateStance(coordinates : dict) -> str:
    """
    Takes an input of keypoint coordinates in the form of a dict and outputs the stance

    Args:
        coordinates (dict): Dictionary of catcher coordinates
...
def ProcessVideo(path, vertical_crop):
    """
    The main logical driver for the video processing
    Takes a video and runs it starting at 3 seconds, then if no coords are found
    runs it starting at 1 second

    Args:
        path(str): path to the video

    Returns:
        The Stance and confidence for the video
    """
...
def findCoordinates(path, vertical_crop, time_crop):
    """
    Calculates the most tracked stance and confidence of the video
    Confidence = tracks of most tracked stance/total valid tracks

    Args:
        path(str): path to input video
        vertical_crop(int): vertical crop
        time_crop(int): how much time to crop the video

    Returns:
        list: [Most common stance, confidence]
    """
    frame_count = 0
    frames_tracked = 0
